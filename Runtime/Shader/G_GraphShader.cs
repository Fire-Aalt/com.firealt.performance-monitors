...
namespace Tayx.Graphy
{
    ...
    public class G_GraphShader
    {
        ...
        public int GetArraySize( int requestedSize )
        {
            return Mathf.Clamp( requestedSize, 1, ArrayMaxSize );
        }
        ...
        {
            var graphValuesLength = ShaderArrayValues == null
                ? 0
                : Mathf.Min( ShaderArrayValues.Length, ArrayMaxSize );

            Image.material.SetInt( GraphValuesLength, graphValuesLength );
        }
        ...
        {
            if ( ShaderArrayValues == null )
            {
                return;
            }

            if ( ShaderArrayValues.Length <= ArrayMaxSize )
            {
                Image.material.SetFloatArray( GraphValues, ShaderArrayValues );
                return;
            }

            if ( m_uploadBuffer == null || m_uploadBuffer.Length != ArrayMaxSize )
            {
                m_uploadBuffer = new float[ArrayMaxSize];
            }

            Array.Copy( ShaderArrayValues, m_uploadBuffer, ArrayMaxSize );
            Image.material.SetFloatArray( GraphValues, m_uploadBuffer );
        }
    }
}
